...
int main(int argc, char **argv) {
  ...
  int num_wires_t=(num_inputs__table-1)*num_wires_input,
        num_wires_n1=num_inputs__table*num_wires_input,num_wires_n2=num_inputs__table*(2*num_wires_input);
  int *inputs_dec__values,*inputs_dec__table_t,*inputs_dec__table_n1,*inputs_dec__table_n2,
        *inputs_bin__values,*inputs_bin__table_t,*inputs_bin__table_n1,*inputs_bin__table_n2,*inputs_bin,
        *outputs_dec,*outputs_bin;
  char file__lookup_table_t[500],file__lookup_table_n1[500],file__lookup_table_n2[500];
  
  int total_num_wires_input__values=num_inputs__values*num_wires_input;                 // total_num_wires_input__values (n_values)
  int total_num_wires_input__table=num_wires_t+num_wires_n1+num_wires_n2;               // total_num_wires_input__table (n_table=n_table_t+n_table_n1+n_table_n2)
  ...
  inputs_dec__table_n1=(int*) malloc(sizeof(int)*num_inputs__table);           // size: n_table_n1/num_wires_input
  inputs_dec__table_n2=(int*) malloc(sizeof(int)*num_inputs__table);           // size: n_table_n2/num_wires_input
  ...
  inputs_bin__table_n1=(int*) malloc(sizeof(int)*num_wires_n1);                // size: n_table_n1
  inputs_bin__table_n2=(int*) malloc(sizeof(int)*num_wires_n2);                // size: n_table_n2
  ...
  printf("  num_wires_t: %d ; num_wires_n1: %d ; num_wires_n2: %d\n",num_wires_t,num_wires_n1,num_wires_n2);
  ...
  printf("  --- look-up table: n2 ---\n");
  for (int i=0; i<num_inputs__table; i++) {
    for (int j=0; j<2*num_wires_input; j++) {
      input_wires__table[(num_wires_t+num_wires_n1)+i*(2*num_wires_input)+j]=(total_num_wires_input__values)+(num_wires_t+num_wires_n1)+i*(2*num_wires_input)+j;
      //printf("    i: %3d ; j: %3d ; input_wires__table: %3d\n",i,j,input_wires__table[(num_wires_t+num_wires_n1)+i*(2*num_wires_input)+j]);
    }
  }
  ...
  MultipleLogsum_RQ(&garbledCircuit,&garblingContext,num_inputs__values,num_wires_input,input_wires__values,
        num_inputs__table,input_wires__table,output_wires,0);
  ...
  writeCircuitToFile(&garbledCircuit,"logsum_RQ.scd");
...
//   }
  ...
  //   look-up table: n1
  sprintf(file__lookup_table_n1,"look-up_table/n1_RQ.num_segments_%ld.scaling_factor_%ld.num_bits_%ld.scale_num_bits_%ld.txt",
        num_inputs__table,(long int)pow(2,num_wires_input),num_wires_input,scale_num_bits);
  ReadInputs(file__lookup_table_n1,num_inputs__table,inputs_dec__table_n1);
  DEC2BIN(num_inputs__table,inputs_dec__table_n1,num_wires_input,inputs_bin__table_n1);
  
  for (int i=0; i<num_inputs__table; i++) {
    for (int j=0; j<num_wires_input; j++) {
      inputs_bin[(total_num_wires_input__values)+(num_wires_t)+i*num_wires_input+j]=inputs_bin__table_n1[i*num_wires_input+j];
    }
  }
  
  //   look-up table: n2
  sprintf(file__lookup_table_n2,"look-up_table/n2_RQ.num_segments_%ld.scaling_factor_%ld.num_bits_%ld+%ld.scale_num_bits_%ld.txt",
        num_inputs__table,(long int)pow(2,num_wires_input+num_wires_input)/8,num_wires_input,num_wires_input,scale_num_bits);
  ReadInputs(file__lookup_table_n2,num_inputs__table,inputs_dec__table_n2);
  DEC2BIN(num_inputs__table,inputs_dec__table_n2,2*num_wires_input,inputs_bin__table_n2);
  
  for (int i=0; i<num_inputs__table; i++) {
    for (int j=0; j<2*num_wires_input; j++) {
      inputs_bin[(total_num_wires_input__values)+(num_wires_t+num_wires_n1)+i*(2*num_wires_input)+j]=inputs_bin__table_n2[i*(2*num_wires_input)+j];
    }
  }
...
//   printf("  --- look-up table: n1 ---\n");
//   for (int i=0; i<num_inputs__table; i++) {
//     printf("    i: %3d ; inputs:  %6d --> ",i,inputs_dec__table_n1[i]);
//     for (int j=0; j<num_wires_input; j++) {
...
//   printf("  --- look-up table: n2 ---\n");
//   for (int i=0; i<num_inputs__table; i++) {
//     printf("    i: %3d ; inputs:  %6d --> ",i,inputs_dec__table_n2[i]);
//     for (int j=0; j<2*num_wires_input; j++) {
//       printf("%d",inputs_bin[(total_num_wires_input__values)+(num_wires_t+num_wires_n1)+i*(2*num_wires_input)+((2*num_wires_input-1)-j)]);
//     }
//     printf("\n");
//   }
